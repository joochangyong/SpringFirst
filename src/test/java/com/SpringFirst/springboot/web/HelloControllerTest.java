...
import com.SpringFirst.springboot.config.auth.SecurityConfig;
...
//테스트를 진행할 때 JUnit에 내장된 실행자 외에 다른 실행자를 실행. 여기서는 SpringRunner라는 스프링 실행자를 사용.
//즉, 스프링 부트 테스트와 JUnit 사이에 연결자 역할.
@RunWith(SpringRunner.class)
//여러 스프링 테스트 어노테이션 중, Web(Spring MVC)에 집중할 수 있는 어노테이션.
//선언할 경우 @controller, @ControllerAdvice등을 사용 가능. 여기서는 컨트롤러만 사용하기 때문에 선언.
...
public class HelloControllerTest {
    ...
    @Autowired
    //웹 API를 테스트할 때 사용. 스프링 MVC 테스트의 시작점. 이 클래스를 통해 HTTP GET,POST 등에 대한 API테스트가 가능.
    private MockMvc mvc;
